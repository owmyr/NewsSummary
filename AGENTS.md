...
- **Gemini free-tier quota**: The free tier is **5 req/min, 20 req/day**. A full pipeline run with `SOURCES=bbc,g1` makes 15-30 API calls. If the quota is exhausted mid-run, some articles will have `summary: "Summary generation failed."`. To recover: wait for the quota to reset (or upgrade), then re-run `python -m daily_bot --retry-failed`. The orchestrator will re-summarize only the failed articles. To stay under the per-minute cap, keep `summarize_concurrency=1`.
